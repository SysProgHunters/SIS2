...
ssize_t readn_str(int fd, char *buf, size_t count)
{
        count -= 1;
        off_t pos = 0;
        while (1) {
...
void cpustats_loadavg(float load[3])
{
...
static void cpustats_subtract1(struct cpustat *out,
...
void cpustats_subtract(struct cpustats *out,
...
static void term_on_sigwinch(int sig)
{
        term_need_resize = 1;
}
...
static void ui_init_panes(int n)
{
        free(ui_panes);
        ui_num_panes = n;
...
void ui_layout(struct cpustats *cpus)
{
        int i;
...
void ui_show_load(float load[3])
{
        char buf[1024];
        int pos;
...
static void ui_show_pane(struct ui_pane *pane)
{
        int row, col;
